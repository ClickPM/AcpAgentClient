...
一个好看的、多 agent 的 ACP 桌面客户端：Tauri 壳，Rust 核心，前端按 Claude Design 设计稿实现；agent 一律经 ACP 接入，registry 里的 agent 像 Zed 一样安装即用，Zed 内置 agent 也能用。
...
5. **前端完全自研。** Claude Design 设计稿是功能边界：设计稿没有的功能不做，设计稿有的逐画板对照实现。
