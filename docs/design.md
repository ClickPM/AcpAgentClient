...
| `registry/progress` | 安装进度 `{agentId, kind, step, done?, total?, detail?, error?, upgrade}`：`kind` ∈ npx / binary / node；npx 的 `step` 依次是 resolve（`npm install` 到 `agents/<id>/<version>/`）/ write_settings / handshake（首次拉起并 `initialize`），binary 是 download（`done` / `total` 字节）/ verify（sha256）/ extract，受管 Node 是 node_download / node_extract（`agentId` 为 null）；收尾一律 done / failed（带 `error`）/ cancelled。`upgrade: true` 是 `registry_update` 的进度（画板 53）：npx 只有 resolve / handshake 两步（settings 条目升级前就在），binary 同上三步，failed / cancelled 时旧版本仍在。前端按 `agentId` 把最近一条落到条目上（画板 51 / 53），速率与剩余时间由前端按 `done` 的时间差估算（R5） |
...
- registry、Node 与设置：`registry_refresh`（1 小时节流，`force` 跳过）、`registry_list`（registry 条目 + settings 里的 custom 条目，各带本地安装 / 认证状态；registry 型已安装条目另带 `updateAvailable`（安装记录的 `version` ≠ registry 当前版本时为当前版本，本平台装不了的不出）与 `reloadPending`（升级后该 agent 仍连着旧版本时为旧版本号，认不出时为空串），画板 53）、`registry_install`（已安装且完整的拒绝）、`registry_update`（画板 53：后台升到 registry 当前版本，见 § 6 第 7 条；没装 / 已是当前版本 / 正在安装都拒绝，取消走 `registry_cancel_install`）、`registry_cancel_install`、`registry_remove`（移除 settings 条目并只删自己写的 `agents/<id>/`）、`node_status`、`node_download`、`agent_settings_get/set/remove`、`agent_settings_import_zed`（R5 补 `agent_settings_remove`：custom 条目从设置页删除，与 `registry_remove` 同一条路径但不碰 `agents/`）
...
- 退出收尾：`core_shutdown`（R4：释放全部终端、断开全部 agent、停掉目录监视；Dart 在 `AppLifecycleListener.onExitRequested` 里等它回来再放行。「全部 agent」含还在握手的 `agent_connect`：中止握手、结束进程树，等它们落定才返回；收尾开始后的 `agent_connect` 回 `cancelled`，iteration-03 第 7 项）
...
- 侧栏只列**当前项目目录**下的会话（2026-09-18，所有者报障）：按索引里的 cwd 与当前项目路径归一后比（分隔符 / 尾斜杠 / Windows 大小写），没记 cwd 的老条目各处都列；换项目时正开着的会话若属于别的目录就从会话区放下（不关、不取消，agent 侧照跑），下一条消息在新目录里现开会话；等待期（`session/new` / 重载在途）里不换项目。
...
- 新建会话**不重连 agent**（2026-09-18）：会话头 `+` 选 agent 与 `send()` 现开会话都走 `ensureConnected`（没连才连），不再断开重拉——另一条会话正在跑的那一轮不会被杀；两个触发共用画板 05 B 组的等待期（`waitingForAgent`）。重载 agent（会话头 reload）仍是断开 + 重拉；重载 / 崩溃之后内存里其它会话的处理见下一条。
- **会话挂在哪条连接上是前端本地态**（iteration-09，`lib/app/session_attach.dart`）：ACP 的会话只属于创建或载入它的那条连接，所以每次 `agent_connect`（重载、崩溃后重连、认证页连接）都把这个 agent 名下内存里的会话标成挂空，切过去或再发（含 Restore / Regenerate / 下拉）时先挂回——声明 `loadSession` 的 `session/load`、否则有 `resume` 的 `session/resume`。发送按四态分流：没有会话 → 现开一条；挂着 → 直接发；挂空 → 先挂回，挂不回**不另开一条顶掉它**，这一轮以失败收在转录里（画板 31 的结束行）；能力已知且挂不回（既没有 load 也没有 resume，或 `session/list` 校对出 agent 侧没有）→ 照 R3 现开一条，输入框占位文案先说明。能力未知时先连上再判。`session/load` 失败时这次重放整段作废，原先有转录的原样留着。
...
7. **升级（画板 53，所有者裁定 2026-09-23）**：手动，不做自动升级。判据是安装记录的 `version`（安装那一刻的 registry 版本）≠ registry 当前版本，只判不等不比大小；不比 `installedVersion`（npm 有界规格在 min-release-age 下可能装到更低的版本）。检查时机：启动、打开 Agents 标签（都走 1 小时节流）、面板标题行的手动检查（`force`）。升级把新版本装进一个新的 `agents/<id>/<version>/`（与在用的入口——安装记录的、正在运行的连接的——撞名时加后缀），npx 装完用新拉起参数做一次握手（这条临时连接不发 `acp/agent_state`：它与运行中的连接同一个 agentId），binary 解压成功即可；通过后写安装记录即切换，认证状态继承、settings 条目与 env 不动。失败 / 取消只删新目录。**不打断运行中的连接**：核心每个 agent 一条连接，升级后它仍跑旧版，安装记录记 `previousVersion`，`registry_list` 据此出 `reloadPending`，会话头 Reload Agent 或重开应用后生效。旧版本目录**一律推迟到下次启动核心**再删（`core_init` 后台扫一遍，扫某个 agent 时占住它的安装槽）：切换那一刻连接表里看不见「正按旧安装记录拉起、还没插回表里」的连接（`agent_connect` 先摘旧连接、connect 完才插回），看不见不等于没人在用（round-board-53 审查 high）；升级完当场只清掉更早留下的、不被任何在用入口用到的目录。
...
- 画板要求、文档原本没有的几项，所有者 2026-09-15 按 `ROUNDS.md` § 6 的推荐一并裁定：
  ...
  - **粘贴复制的文件与目录**（所有者 2026-09-23）：同一次读取里 `CF_HDROP` 的路径**默认按路径引用**——加成 `resource_link`、正文里一个 `@名字`，与 `@` 菜单挑中同一个形状，项目外的文件与目录只有这条路加得进来；只有「agent 收图、图片扩展名、不超 20 MB」的文件才读成 `image` 块，空的与超限的图片文件也退回路径。agent 不收图时通道参数 `{"bitmap": false}` 让 runner 不取位图。`+` → Files & Directories 同日改成往输入框插 `@` 弹画板 42 的菜单（照 Zed），不再弹原生文件对话框（选文件那种点目录只会进到下一级）。
  ...
  - **会话时间线**（画板 43，2026-09-20）：会话头 reload 与 ≡ 之间一个 history 按钮，弹层按轮列「用户 query 首行 + 该轮最终回答首行（A）」，点一行转录区跳到那一条。轮的切分按**顶层用户消息**而不是 `TurnEntry`——轮边界只有客户端自己 `session/prompt` 时才放，`session/load` 重放回来的历史一条都没有（Restore 截断点踩过同一个坑）。整份是 `SessionStore.entries` 的纯派生，不新增 `docs/acp-projection.md` § 7 的自造态、不碰协议。**回合折叠（画板 08）2026-09-22 起同口径**：有 `TurnEntry` 就按轮边界（要它身上的模型名与 `stopReason`），没有就退到顶层用户消息，重放回来的历史因此也能折；历史轮没有轮边界，摘要行退化成单行、回合页脚不出（`design/DIVERGENCE.md` A-16）。
