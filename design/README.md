...
| 06 | 侧栏会话活动指示 | 会话工作台（侧栏） | round-design | `design/round-design/06-session-activity.dc.html` | `design/round-design/06-session-activity.png` | — | 已实现（2026-09-18，main 直改）· 2026-09-23 D 表「等待授权 / 追问」一行改指画板 09（已实现，随画板 09） | 新增 `Sweep`（track / focus / focusGradient / cycle / band / inset / bottom）与 `UnreadDot`（size / color / gap）两组；`Geometry` 补 `sidebarRowRunning` 58 与 `sidebarRowRunningContent` 50 |
...
| 08 | 交互增强（回合折叠 / 跨工作区在跑数） | 会话工作台（转录 + 顶栏项目切换器） | round-design | `design/round-design/08-interaction-upgrades.dc.html` | `design/round-design/08-interaction-upgrades.png` | — | 已实现（2026-09-22，`claude/new-session-c0ff9d` 分支）·**A 段设计阶段已删除，不实现** · 2026-09-23 C 段口径「什么算在跑」改为不含等你处理的会话（已实现，随画板 09） | 新增 `Fold`（rowPadding / radius / bg / hover / secondLineIndent / lineGap / headerPadding / itemGap）与 `Badge`（accentBg / height / radius / padding / gap / iconSize / iconStroke / overflowAt）两组；`badge.accent.bg` 由 `Accent.base` 现算（照画板 07 对 `Sweep.focusGradient` 的做法），画板 00 不改 |
| 09 | 等你处理（侧栏条目 / 工作区切换器） | 会话工作台（侧栏 + 顶栏项目切换器） | round-design | `design/round-design/09-awaiting-you.dc.html` | `design/round-design/09-awaiting-you.png` | https://claude.ai/design/p/3add6fd9-05b9-4cbd-9892-dec110c50983?file=09-awaiting-you.dc.html | 已实现（board-09，`claude/permission-request-badge-missing-c4a919` 分支，2026-09-23） | 无新设计 token；实现侧收一组 `AwaitingMark`（color / iconSize / iconStroke / iconGap / gap），`Badge` 补 `warningBg` / `warningFg` / `pairGap`，全部引用既有的 `Semantic.warning` / `warningSoft`、`IconSizes.stroke`、`Spacing.s4`、`UnreadDot.gap` |
...
- 2026-09-23 新增画板 53「Registry 升级态」（1200 × 700，page-4 放在画板 52 右边 `x 4080`），并改画板 50 三处：标题行在 `Learn More` 左边加「检查于 N 分钟前」与刷新按钮（`rotate-cw`，悬停提示「检查更新」）、Codex 行换成可升级态（`v1.11.0 → v1.12.0` + `可升级` 芯片 + `Update` / `Remove`）、底部注释追加可升级的判据。起因是 registry 型 agent 装好之后没有检查与升级（BACKLOG「装好的 agent 没有升级入口」）；所有者裁定：手动升级不做自动升级、判据是「安装记录里的 registry 版本 ≠ registry.json 当前版本」只判不等、新版本装在旧版旁边握手通过才切换、不打断正在运行的连接、提示只在 Agents 面板条目上。画板 53 画了五样：标题行「检查中」、有新版本、升级中 · npx（两步 + 「旧版仍可用」小字）、升级失败（旧版不受影响）、已升级 · 待重载（运行中的连接仍是旧版）。**画板 51 不动**：它下方 120 px 就是画板 61，往里加卡片要挪整页。**画板 50 没有整份拉回**：画布那份的 `$preview` 高度是 1000、本地是 1005（本地自 `72e2be1` 未改过），只把三处改动按画布的写法插进本地文件，PNG 尺寸不变。无新 token、无新图标。简报 `design/round-design/input/revision-06.md`。
