...
- **2026-09-22 发布 v1.4.1**：v1.4.0 之后合入 main 的六批改动的**复审与整改版**——六批是 quality 代码质量清理、审查模型换代、画板 43 时间线跳转白屏修复、添加工作目录 / 切项目假死修复、cursor 九条 findings 的验真与修复、画板 08 回合折叠 + 跨工作区在跑数 + 画板 70「转录」开关；其中三批此前从未经过独立审查。这一版先由 Claude 自主审查一遍（20 条 findings 逐条处理，含 2 条 high：删掉再新建同 id 的会话会被本地索引静默删掉；画板 70 的开关因读盘排在 `core_init` 之前而既读不回也存不下），再经 cursor 复审两轮归零，细目见 `rounds/round-1.4.1/round-1.4.1.md`。打包与安装器沿用 v1.4.0 的流程（`scripts/package.ps1` / `verify-package.ps1`），产物以 release 页为准。
