...
**执行器**（所有者裁定 2026-09-11，与 agent-xray 一致）：独立审查由 **cursor CLI（`cursor-agent`）+ `cursor-grok-4.6-high`** 执行，codex 不可用。
...
- 依赖白名单是硬规则（CLAUDE.md 规则 1）：出现白名单之外的 ACP 客户端、agent 状态或会话 UI 库，或引入 UI 组件库，判**阻断级**；通用库允许清单之外的新增没写理由判一般级。
