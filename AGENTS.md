...
**执行器**（所有者裁定 2026-09-11，与 agent-xray 一致）：独立审查首选 **cursor CLI（`cursor-agent`）+ `cursor-grok-4.6-high`**；
...
- 依赖白名单是硬规则（CLAUDE.md 规则 1）：出现白名单之外的 ACP 客户端、agent 状态或会话 UI 库，或引入第三方 UI 组件库 / 状态管理库，判**阻断级**；通用库允许清单之外的新增没写理由判一般级；Markdown 渲染库在所有者裁定进清单之前出现在 `pubspec.yaml` 判一般级。
...
- 审查范围口径：前两轮全量分支 diff，第 3 轮起只审上一轮整改 diff；范围之外的既有问题不报，记 BACKLOG。
