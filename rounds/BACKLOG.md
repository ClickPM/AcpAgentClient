...
| **P1 看得见的粗糙** | 7 | 用户看得见的不一致、缺等待态、行为不符直觉。能用，膈应；攒批做。 |
...
| P5 内部工程与验收 | 1 | 测试、行数门、验收自动化这类用户无感的问题。原先 16 条已于 2026-09-23 整档收掉（iteration-04），见下面的小节 |
...
- [ ] **认证期间点开了另一条会话，认证成功后被新会话顶掉**
  - **产品**：发第一条消息撞上登录、认证页开着的时候在侧栏点了同一项目里的另一条会话，登录一成功，新建出来的会话把刚点开的那条顶掉。
  - **技术**：认证页成功后的自动重试（`AuthState.start` → `onAuthenticated` → `SessionController.createSession` / `adoptAuthSession`）在会话 cwd 属于当前项目时一律切成当前会话；iteration-07 只补了「认证期间换了项目」那一半（`_adoptSession` 判 `workspace.inCurrentWorkspace`）。同一项目里中途切了会话的，要在认证发起时记下当时的选中态（如 `sessionEpoch`）、成功时比一次，没变才切——认证页有三个入口（`-32000`、画板 51 / 34 的登录键），要一并带上这个记号，属机制类，没在本迭代顺手做 (iteration-07, 2026-09-23)
...
- [ ] **点开旧会话时没有「正在载」**
  ...
  - **技术**：第三条「等 agent」的路径还没有等待态：侧栏点一条内存里没有转录的会话（`selectSession` → `ensureLoaded`（`lib/app/session_attach.dart`）→ `session/load` 重放整段历史）。`sessionId` 当帧就切过去了，而转录要等重放回来，这期间画的是画板 01 的**新会话空态**——看起来像「这条会话是空的」，不是「正在载」。所有者手测只报了新建会话那条，这条一并记下。要修就是同一个 `waitingForAgent` 套在 `_ensureLoaded` 的 `_guard` 上，但画板 05 A 组把「侧栏点另一条会话」定义成瞬时替换、没画等待期，属扩边界，先改设计稿 (2026-09-18) → **R7.5 拆分后的新家**：`SessionController._ensureLoaded`（`waitingForAgent` 套上去；画板 05 A 组先补等待期） (2026-09-20)
