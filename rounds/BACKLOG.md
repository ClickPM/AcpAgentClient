...
| 档 | 条数 | 这档是什么 |
|---|---|---|
| **P0 真缺陷** | 4 | 会丢内容、作用到错对象、吃光资源、静默失败。撞上就是事故，排进最近的轮次。 |
...
2026-09-23 全仓只读审查（v1.4.4 之后的 `main`，Rust 核心 / 文件与终端 / Dart 状态层 / 投影与转录四路）登记了其中 7 条（「本地状态文件的读改写没有串行化」已由 iteration-10 修掉，「资源与静默失败」一小节三条已由 iteration-11 修掉，余 3 条），代码路径都逐条核过，均未在 Windows 实机复现；「dsh 的会话存到哪里跟着进程工作目录走」来自所有者同日报障，根因由读代码推出，待实机确认。
