...
- [ ] 终端面板（画板 61）改走 `hardwareKeyboardOnly` 之后没有 IME 组字：xterm 的平台文本输入通道在 Windows 引擎上建不起来（`TextInput.setClient` 缺 `viewId` 被直接拒，见 `lib/ui/terminal/terminal_panel.dart` 的注），只能取 `KeyEvent.character`，中文输入法在终端里打不出字（ASCII 与粘贴不受影响；转录里的终端卡与 terminal auth 本来就是这条路）。要恢复组字得等 xterm 在 `TextInputConfiguration` 里带上 `viewId`（升级它属于改依赖版本），或自写一层带 `viewId` 的文本输入代理 (2026-09-17)
