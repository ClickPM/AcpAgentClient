...
- [ ] round-design 子代理卡（画板 24）依赖 `_meta.claudeCode.subagent`，与规则 2「无 agent 特判」及 `docs/design.md` § 4 的 `_meta` 键清单有张力；设计照原型出，实现前裁定 (2026-09-14)
- [ ] round-design 文件树的 git 状态徽章（原型里的 `M`）不在任何文档里；设计照原型出，实现前裁定是否保留 (2026-09-14)
