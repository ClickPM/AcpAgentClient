...
// 覆盖为 0 的纯拉丁字体，否则它会把中文也吃掉、中文轴就失效了（理由见 `lib/app/font_prefs.dart` 文件头）。
//
...
class AppearanceCard extends StatefulWidget {
  const AppearanceCard({super.key, required this.fonts, this.onOpenUrl});
...
class _AppearanceCardState extends State<AppearanceCard> {
  ...
  void _pick(FontAxis axis, FontChoice choice) {
    _handles[axis]!.hide();
    setState(() => _open = null);
    widget.fonts.setAxis(axis, choice.family);
  }

  Widget _menuFor(FontAxis axis) {
    final List<FontChoice> choices = fontCatalog[axis] ?? const <FontChoice>[];
    final String current = widget.fonts.prefs.resolved(axis);
    return MenuPopover(
      children: <Widget>[
        for (final FontChoice c in choices)
          MenuTwoLineRow(
            title: c.label,
            meta: widget.fonts.registry.isAvailable(c) ? (c.note ?? '') : '本机未找到 · ${c.note ?? ''}',
  ...
  Widget _row(FontAxis axis, {required bool bottomBorder}) {
    ...
    final bool available = choice == null || widget.fonts.registry.isAvailable(choice);
