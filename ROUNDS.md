...
| 画板 53（registry 版本检查与升级） | 进行中 | `registry-upgrade`（独立 worktree `AcpAgentClient-upgrade`，从 `main@8b56dbc` 开出） | `458815d`（画板 53 入库 + 画板 50 三处改动） | — | — | 任务卡 `rounds/round-board-53/round-board-53.md`；简报 `design/round-design/input/revision-06.md`。改 `docs/design.md` § 3 契约（`registry_update` + `registry_list` 两个字段 + `registry/progress` 的 `upgrade`），所以走轮次不走迭代 |
