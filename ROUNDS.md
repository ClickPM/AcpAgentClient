...
| 06 | 侧栏会话活动指示 | main 直改（2026-09-18） | `sidebar.dart`（`SessionSweepLine` / `SessionUnreadDot` + 会话项的 running / unread 两态）+ 接线点 `session_controller.dart`（`activity` / `unreadSessionIds`；在跑集合 2026-09-23 起由 `lib/projection/session_activity.dart` 现算，见画板 09）、`workbench_screen.dart`；数值在 `tokens.dart` 的 `Sweep` / `UnreadDot` / `Geometry` |
...
| 08 | 交互增强（回合折叠 / 跨工作区在跑数） | `claude/new-session-c0ff9d` 分支（2026-09-22） | B：`lib/projection/turn_fold.dart`（分组规则）+ `lib/ui/transcript/turn_fold_row.dart`（摘要行）+ `lib/app/transcript_folds.dart`（全局开关 + 每回合展开态）+ `transcript_list.dart`（行装配与滚动锚点）+ `entries.dart` / `session_store.dart`（`TurnEntry.model` 快照）+ `settings_page.dart`「转录」分组+ `rust/settings` 的 `Transcript` 段与 `transcript_prefs_get/set`；C：`lib/ui/shell/running_badge.dart` + `topbar.dart` + `topbar_popovers.dart` + `lib/projection/session_activity.dart`（`runningByWorkspace` / `runningTotal`，2026-09-23 随画板 09 从 `session_controller.dart` 挪过去）。**A 段（token 速度标签）设计阶段已删除，不实现** |
| 09 | 等你处理（侧栏条目 / 工作区切换器） | `claude/permission-request-badge-missing-c4a919` 分支（2026-09-23，iteration-08） | 派生在 `lib/projection/session_activity.dart`（在跑 / 等你处理互斥与按工作区计数）+ `pending.dart` 的 `firstBySession`；A：`sidebar.dart`（`SessionAwaitingMark` + 等你期间只画底线的 `_SweepPainter.trackOnly`）；B：`running_badge.dart`（`AwaitingBadge` / `ActivityBadges`）+ `topbar.dart` + `topbar_popovers.dart`；接线点 `session_controller.dart`（`activity`）、`workbench_screen.dart`；数值在 `tokens.dart` 的 `AwaitingMark` / `Badge` |
...
| 50 | Agents 面板（ACP Registry） | R5；标题行检查更新与 Codex 可升级态随画板 53（`registry-upgrade`，2026-09-23） | `lib/ui/registry/registry_panel.dart`（标题行拆成 `RegistryPanelHeader`） |
| 51 | Registry 条目状态 | R5 | `lib/ui/registry/registry_entry.dart` |
| 52 | agent 认证 | R5 | `lib/ui/registry/auth_page.dart` |
| 53 | Registry 升级态 | `registry-upgrade` 分支（2026-09-23） | 条目四态在 `lib/ui/registry/registry_entry.dart`，标题行「检查中」在 `registry_panel.dart` 的 `RegistryPanelHeader`；状态层 `lib/projection/registry.dart`；核心 `rust/acp-core/src/registry_ops.rs`（`registry_update`）+ `rust/registry/src/install.rs`（按版本分目录、清旧目录） |
...
前端其余目录（R0 定型）：`lib/app/`（组合根：R3 落 `workbench_controller.dart` 状态与动作、`workbench_screen.dart` widget 装配、`window_controls.dart` 平台通道、`headless_run.dart` 无头实跑；R4 加 `files_state.dart` 与 `local_terminals.dart`；2026-09-18 加 `clipboard_image.dart`（剪贴板图片；2026-09-20 quality 轮起由 runner 走 Win32 读，`windows/runner/acp_clipboard.cpp`）；数据源选择 fixtures / bridge；R7.5（2026-09-20）把 `workbench_controller.dart` 拆成组合根 + 8 个对象：`shell_state` / `workspace_state` / `agents_state` / `auth_state` / `composer_state` 按画板分组管本地态，`session_controller` / `turn_controller` 驱动协议，`session_index` 是本地索引镜像，`guarded` 是共用的通知与错误边界；依赖方向见 `rounds/round-7.5/round-7.5.md` 附录 B）、`lib/bridge/`（frb 生成物，入库）、`lib/projection/`（投影状态层，纯 Dart，无 widget 依赖）、`lib/theme/tokens.dart`、`lib/gallery/`（画板对照；不进产品入口，由 `test/gallery_test.dart` 离屏渲染到 `build/gallery/`）。
...
| 1.4.1 复审（v1.4.0 之后六批合并的复审与整改） | 已完成，2026-09-22 快进合入 `main`；待发布 v1.4.1 | `claude/review-1.4.1`（从 `main@689fab5` 开出） | —（无画板阶段） | `f1fb10c`（2026-09-22 快进合并，无 merge commit） | 第 1 遍 **Claude 自主审查**（所有者指定；4 个 opus 只读子代理并行 + 主会话通读六批 diff 逐条验真）：20 条 high 2 / P2 5 / P3 13，16 条采纳整改（含 2 条 high 与 5 条 P2 全部）、1 条不采纳（tar 的 GNU 语义，写明理由）、1 条只改注释、2 条记 BACKLOG；第 2 遍 cursor CLI `grok-4.7-high-fast` 全量 `v1.4.0..HEAD` 两轮：第 1 轮 2 条（P2 1 / P3 1，都指向整改本身）采纳整改，第 2 轮 **0 条**收口 | 任务卡 `rounds/round-1.4.1/round-1.4.1.md`。所有者指示审查期间不跑构建。范围是 `v1.4.0..689fab5` 的六批：quality（cursor 3 轮过）、审查模型换代、画板 43 跳转白屏（**未审查**）、切项目假死（**未审查**）、cursor 九条修复（**未审查**）、画板 08（cursor 4 轮过）。high ①（cursor 九条修复那批）会话索引的墓碑永不过期，删掉再新建同 id 的会话（fake-agent 不带 `--sessions` 恒回 `sess_fake_1`）被静默删掉、侧栏不出现 → 删除改为只等该会话在途的 upsert 落地、删除在途期间不发同会话的写；high ②（画板 08 那批）转录偏好的读盘排在 `core_init` 之前且无补读，画板 70 的开关既读不回也存不下 → 挪到 init 之后。P2：换项目补齐窗口里旧项目的分支表仍可点、会对新项目跑 `git switch`；URL 型 elicitation 卡不认 withdrawn；`resolve_inside` 用例注释声称覆盖了走不到的分支；画板 70 全局开关翻面没有滚动锚点（锚点订阅 `TranscriptFolds` + 量不到时交 `TranscriptJump` 分帧找回，顺带收掉 BACKLOG 的「锚点量不到不校正」残余）；压缩标记用例建的其实是 tool_call。validate 全绿（`flutter test` 401 项，新增 8 条用例，high / P2 的整改逐条验证过去掉整改会红）；runner 的一行 C++（`return !out.empty()`）未编译，随 v1.4.1 的 release 构建一起编。版本号 1.4.0 → 1.4.1 随收口一起改（sidecar 不跟，规则 11） |
| 画板 53（registry 版本检查与升级） | 已完成，2026-09-23 快进合入 `main`（所有者指示） | `registry-upgrade`（独立 worktree `AcpAgentClient-upgrade`，从 `main@8b56dbc` 开出） | `458815d`（画板 53 入库 + 画板 50 三处改动） | `c29ec88`（先把 `main@d659e42` 合进分支、只有 BACKLOG 两处文档冲突，再快进；合并后 validate 16 项全绿、456 项测试，无代码改动不复审） | 3 轮 / cursor CLI `grok-4.7-high-fast`（第 1–2 轮全量 `main...HEAD`：3 条 high 1 / P2 1 / P3 1，2 条 P2 1 / P3 1；第 3 轮只审整改 diff `04b9af2..HEAD`：**0 条**；high「切换时删掉正在拉起的连接要用的旧目录」与 P2「认证回写与切换互相覆盖 install.json」都已整改） | 任务卡 `rounds/round-board-53/round-board-53.md`；简报 `design/round-design/input/revision-06.md`。改 `docs/design.md` § 3 契约（`registry_update` + `registry_list` 两个字段 + `registry/progress` 的 `upgrade`），所以走轮次不走迭代 |
| 1.4.3 复审（v1.4.2 之后合入 main 的代码改动的复审与整改） | 已完成，2026-09-23 快进合入 `main` 并发布 v1.4.3 | `claude/review-1.4.3`（在 release worktree 上从 `main@64a83d1` 开出） | —（无画板阶段） | 快进合并，无 merge commit | 第 1 遍**主会话自主审查**（所有者指定，不委派子代理；文档改动不审不改）：62 个代码文件，high 0 / P2 0 / P3 4，采纳 1（`install_npx` 清空残留目标目录改走阻塞线程）、3 条写明理由不整改；第 2 遍 cursor CLI `grok-4.7-high-fast` 全量 `v1.4.2..HEAD` 两轮：第 1 轮 1 条 P2（Dart `$` 语义误判，不采纳、补用例锁住行为），第 2 轮 **0 条**收口 | 任务卡 `rounds/round-1.4.3/round-1.4.3.md`。范围六批：iteration-03 第 1 / 2 项与第 3 项（**未审查**，所有者指定）、画板 53（cursor 3 轮过）、iteration-03 第 4 / 5 项（cursor 3 轮 + 全量 1 轮过）、iteration-04（cursor 1 轮过）、iteration-03 第 6 项（cursor 1 轮过）。validate 全绿（`flutter test` 462 项）。版本号 1.4.2 → 1.4.3（sidecar 不跟，规则 11） |
| 1.4.4 复审（v1.4.3 之后合入 main 的代码改动的复审与整改） | 已完成，2026-09-23 快进合入 `main` 并发布 v1.4.4 | `claude/review-1.4.4`（在 release worktree 上从 `main@571b828` 开出） | —（无画板阶段） | 快进合并，无 merge commit | 第 1 遍**主会话自主审查**（所有者指定，不委派子代理；文档改动不审不改）：45 个代码文件，high 0 / P2 0 / P3 3，采纳 1（外观落盘失败写 `lastError` 并接到壳级 toast）、2 条写明理由不整改；第 2 遍 cursor CLI `grok-4.7-high-fast` 四轮：第 1 轮全量 `v1.4.3..HEAD` 2 条 high（`session/new` 回已有 id 时旧转录留在新会话上；关窗 8 秒超时盖不住收尾最坏一段 12 秒——主会话那条「不整改」P3 的反例）全采纳，第 2 轮全量 1 条 high（重载断开途中再点当前会话，「已在载」被当成失败新开一条）采纳，第 3 轮只审整改 diff 1 条 P2（用例顺序质疑，两处实测推翻，不采纳），第 4 轮 **0 条**收口 | 任务卡 `rounds/round-1.4.4/round-1.4.4.md`。范围：iteration-05（终端组字）、06（通用 toast、主题跟随系统）、07（画板 09 等你处理）、09（会话挂载四态）、iteration-03 第 7 项（退出回收 agent 子进程），此前各自 cursor 0 条收口。validate 三次全绿（`flutter test` 527 项）。版本号 1.4.3 → 1.4.4（sidecar 不跟，规则 11） |
| dsh-1.3.2（dsh 会话根目录移出进程工作目录：上游修 + 升钉版本） | 已完成，2026-09-24 快进合入 `main`（所有者指示；未构建） | `claude/backlog-dsh-upstream-e32363`（从 `main@7dcdfbe` 开出） | —（无画板阶段） | 快进合并（先把 `main@4f1c057` 合进分支：只有 BACKLOG 两份登记文档冲突，代码自动合并，无代码改动不复审） | 1 轮 / cursor CLI `grok-4.7-high-fast`（全量 `main...HEAD`）：**0 条** | 任务卡 `rounds/round-dsh-1.3.2/round-dsh-1.3.2.md`；上游 ClickPM/dsh-acp-interactive@e39fd48（npm 1.3.2）；关闭 BACKLOG P0「dsh 的会话存到哪里跟着进程工作目录走」；旧 `.sessions` 不动（所有者指示） |
| send-queue（回合进行中的发送队列） | 未开始（2026-09-24 立项，文档先行） | —（开工时建） | —（设计阶段未开始：简报拟 `design/round-design/input/revision-08.md`，画板编号届时查重再定） | — | — | 任务卡 `rounds/round-send-queue/round-send-queue.md`；照 Zed 的 Queued Messages 状态机（依据 `docs/research.md` § 4.2）在客户端本地排队，只用标准 `session/prompt` / `session/cancel`，核心与桥零改动。所有者裁定 2026-09-24：**不做 steer**——不接各家 agent 的 `_session/steering` 私有扩展、不为 Zed sidecar 做 steer 适配，调研记录在 `docs/research.md` § 4.3；另有 a–h 八项开工前裁定（队列归属 / 持久化 / 断开与出错时的处理 / 位置与键位等）列在任务卡 |
