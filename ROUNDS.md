...
| dsh-1.3.2（dsh 会话根目录移出进程工作目录：上游修 + 升钉版本） | 进行中：上游 1.3.2 已发布，本项目改动待审查 | `claude/backlog-dsh-upstream-e32363`（从 `main@7dcdfbe` 开出） | —（无画板阶段） | — | 待审 | 任务卡 `rounds/round-dsh-1.3.2/round-dsh-1.3.2.md`；上游 ClickPM/dsh-acp-interactive@e39fd48（npm 1.3.2）；关闭 BACKLOG P0「dsh 的会话存到哪里跟着进程工作目录走」；旧 `.sessions` 不动（所有者指示） |
