...
| R7.5 | 进行中 | `session-timeline` | — | — | — | 会话记录（画板 43）；与 R7.6 并行，两边都动了 `lib/theme/tokens.dart`，后合并的一方按「并行轮次合并」先合 `main` 解冲突再全量复审 |
| R7.6 | 进行中（实现与自验完成，待审查） | `font-switching`（worktree `AcpAgentClient-fonts`） | —（设计稿待补，见下） | — | — | 字体切换四轴（界面西文 / 界面中文 / 代码等宽西文 / 代码等宽中文），所有者裁定 2026-09-20「字体属聚合物 + 随包直选 + 两组互不重叠的下拉」；任务卡 `rounds/round-7.6/round-7.6.md`；validate 全绿（277 测试）；**画板 70 的「外观」小节属实现先行、设计稿待补**；随包字体文件需所有者本人下载后放 `assets/fonts/optional/`（协议的点击同意不可由工具绕过），在此之前验收 9 待完成 |
