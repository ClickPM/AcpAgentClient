...
| `26-terminal-meta.jsonl` | （R4）画板 22 / 23 的另一条数据源：`tool_call_update._meta.{terminal_info, terminal_output, terminal_exit}`（Zed 读的终端 provider 通道；钉版本的 claude-agent-acp / dsh / codex-acp 都走它、不调 `terminal/create`）——追加语义、退出码或信号二选一；所有者裁定待确认（`docs/design.md` § 4） |
