...
void main() {
  testWidgets('线程头 + 选完 agent：会话还没到手就先进等待态（转录变暗不可点 + 线程头 spinner）', (tester) async {
    ...
    expect(c.thread.sessionId, isNull, reason: '门还没开，会话确实还没建出来');
    ...
    expect(c.thread.sessionId, 'sess_fake');
  ...
  testWidgets('新会话在途时再选一次 agent：第二条被守卫挡住，只拉一次进程，等待态在会话到手后收掉', (tester) async {
    ...
    await tester.pump(t.Motion.transition);
    expect(c.thread.sessionId, 'sess_fake');
    expect(c.thread.waitingForAgent, isFalse, reason: '等待态不能被交叠的第二条留在 true');
  });
}
