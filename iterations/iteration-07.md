# Iteration 07 — 等你处理：后台 / 换走的会话挂着权限或表单请求时看得见（画板 09）
...
- 合并后：BACKLOG P1「数据一致性」这两条压成一行剪到 `rounds/BACKLOG-CLOSED.md`（写 `→ iteration-07`），并重算 BACKLOG 档位合计（`main` 上的合计与本分支开出时不同，按真实条数算）。
